/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   ft_uctostr.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: vsempere <marvin@42.fr>                    +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/11/17 14:09:06 by vsempere          #+#    #+#             */
/*   Updated: 2020/04/25 22:51:51 by vsempere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "libft.h"

static size_t		ft_numberlen(unsigned char n, int base)
{
	size_t			s;

	s = 1;
	while (n >= base)
	{
		n = n / base;
		s++;
	}
	return (s);
}

char				*ft_uctostr(unsigned char n, int base, char lower_case,
	char group_with_thousands)
{
	char			*digits;
	size_t			pos;
	char			*nstr;
	size_t			numberlen;

	numberlen = ft_numberlen(n, base);
	if (!(nstr = ft_calloc(numberlen +
		(!group_with_thousands ? 0 : numberlen % 3) + 1, sizeof(char))))
		return (0);
	digits = (lower_case ? "0123456789abcdef" : "0123456789ABCDEF");
	pos = numberlen - 1;
	while (n >= base)
	{
		if (group_with_thousands && !(pos % 3) && pos != (numberlen - 1))
			nstr[pos--] = THOUSANDS_SEPARATOR;
		nstr[pos--] = digits[(n % base)];
		n = n / base;
	}
	nstr[pos] = digits[(n % base)];
	return (nstr);
}
